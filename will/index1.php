...
if (!isset($_SESSION['benname'])) {
    header("Location: index.html");
}else{
  $filter = $_SESSION['benname'];
...
<head>
  ...
  <title>Will Platform - Beneficiary Homepage</title>
    ...
    <header class="header_section">
      <div class="container">
        <nav class="navbar navbar-expand-lg custom_nav-container pt-3">
          <a class="navbar-brand mr-5" href="index1.php">
            <span>
              Will Platform
            </span>
          </a>
...
<?php
$sql = "SELECT * FROM `will` WHERE `Beneficiary_ID` = '$filter'";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
// output data of each row
while($row = $result->fetch_assoc()) {
